...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { abbr: @local.abbr, name: @local.name, parent_id: @local.parent_id, type: @local.type }
    end
  ...
  test "should update local" do
    patch :update, id: @local, local: { abbr: @local.abbr, name: @local.name, parent_id: @local.parent_id, type: @local.type }
    assert_redirected_to local_path(assigns(:local))
  end
